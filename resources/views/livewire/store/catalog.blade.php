<div>
    ...
    <!-- Contenedor principal con detección de scroll -->
    <div class="min-h-screen bg-[#050505] text-white font-sans selection:bg-[#D4AF37] selection:text-black overflow-x-hidden relative"
        ...
        <!-- ========================================== -->
        <!-- HEADER / NAVEGACIÓN PROFESIONAL (INTACTO) -->
        <!-- ========================================== -->
        ...
        <!-- ========================================== -->
        <!-- BOTÓN FLOTANTE GIGANTE Y VISTA DE CARRITO -->
        <!-- ========================================== -->
        <div x-cloak 
             x-show="isScrolled" 
            ...
            <div x-data="{ openCart: false }" class="relative" @click.away="openCart = false">
                ...
                <!-- Vista del carrito (Panel grande) -->
                <div x-cloak
                     x-show="openCart"
        ...
        <main class="relative z-10 max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 pb-32 space-y-32">
            ...
            @else
                ...
                <!-- 1. COLECCIÓN EXCLUSIVA (Carrusel Original) -->
                <section x-data="{ 
                    ...
                    <div x-ref="carouselExclusiva" class="flex gap-6 overflow-x-auto snap-x snap-mandatory hide-scroll pb-10">
                        <!-- Reemplaza $products con tu variable de exclusivos si es necesario -->
                ...
                <!-- 2. OFERTAS ESPECIALES (Nuevo Carrusel con Etiquetas de Descuento) -->
                <!-- Nota: Estoy usando $products como ejemplo, idealmente debes enviar $ofertas desde tu componente Livewire -->
                <section x-data="{ 
                    ...
                    <div class="flex items-end justify-between mb-8">
                        <h2 class="text-3xl md:text-5xl font-light uppercase tracking-widest border-l-2 border-[#D4AF37] pl-4">
                            Ofertas <span class="font-bold text-[#D4AF37]">Especiales</span>
                        </h2>
                        <div class="hidden md:flex gap-4">
                            <button @click="scrollPrev" class="w-12 h-12 rounded-full border border-white/20 flex items-center justify-center hover:bg-white hover:text-black transition-all text-white">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
                            </button>
                            <button @click="scrollNext" class="w-12 h-12 rounded-full border border-white/20 flex items-center justify-center hover:bg-white hover:text-black transition-all text-white">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                            </button>
                        </div>
                    </div>
                    ...
                    <div x-ref="carouselOfertas" class="flex gap-6 overflow-x-auto snap-x snap-mandatory hide-scroll pb-10">
                        @foreach($products->take(4) as $product)
                            <div class="min-w-[280px] md:min-w-[320px] snap-center group relative rounded-2xl bg-[#0A0A0A] border border-white/5 hover:border-white/30 transition-all duration-500 overflow-hidden shadow-2xl">
                                <!-- Etiqueta roja/dorada de OFERTA -->
                                <div class="absolute top-4 right-4 z-20 bg-red-600/90 backdrop-blur-md text-white text-[10px] font-black px-3 py-1.5 uppercase tracking-widest rounded shadow-lg">
                                    Oferta
                                </div>
                                <div class="h-[300px] w-full overflow-hidden relative bg-black">
                                    @if($product->image)
                                        <img src="{{ \Illuminate\Support\Facades\Storage::url($product->image) }}" alt="{{ $product->name }}" class="w-full h-full object-cover object-center group-hover:scale-105 transition-transform duration-700 opacity-90 group-hover:opacity-100">
                                    @endif
                                </div>
                                <div class="p-6 text-center">
                                    <h3 class="text-lg font-bold uppercase tracking-widest text-white truncate">{{ $product->name }}</h3>
                                    <div class="flex justify-center items-center gap-3 mt-2 mb-6">
                                        <p class="text-sm text-gray-500 line-through">${{ number_format($product->retail_price * 1.2, 2) }}</p>
                                        <p class="text-2xl font-black text-white">${{ number_format($product->retail_price, 2) }}</p>
                                    </div>
                                    <button wire:click="addToCart({{ $product->id }})" class="w-full bg-white text-black py-3 tracking-[0.2em] text-xs font-bold uppercase transition-all hover:bg-[#D4AF37]">
                                        Aprovechar Oferta
                                    </button>
                                </div>
                            </div>
                ...
                <!-- 3. CATÁLOGO COMPLETO (Grid de Productos) -->
                <section>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 sm:gap-8">
                        @foreach($products as $product)
                            <div class="group relative rounded-2xl bg-white/5 border border-white/10 hover:border-[#D4AF37]/50 transition-all duration-500 overflow-hidden backdrop-blur-sm shadow-lg hover:-translate-y-1">
                                <div class="h-[280px] w-full overflow-hidden relative bg-black/50">
                                    @if($product->image)
                                        <img src="{{ \Illuminate\Support\Facades\Storage::url($product->image) }}" alt="{{ $product->name }}" class="w-full h-full object-cover object-center group-hover:scale-110 transition-transform duration-700 ease-out opacity-80 group-hover:opacity-100">
                                    @endif
                                </div>
                                <div class="p-5 text-center">
                                    <p class="text-[10px] text-gray-400 uppercase tracking-widest mb-1">{{ $product->brand ?? 'Sillage' }}</p>
                                    <h3 class="text-base font-bold uppercase tracking-wider text-white truncate">{{ $product->name }}</h3>
                                    <p class="text-xl font-light text-[#D4AF37] mt-2 mb-5">${{ number_format($product->retail_price, 2) }}</p>
                                    
                                    <button wire:click="addToCart({{ $product->id }})" class="w-full bg-transparent border border-white/20 text-white py-2.5 tracking-[0.2em] text-[10px] font-bold uppercase transition-all hover:bg-[#D4AF37] hover:border-[#D4AF37] hover:text-black">
                                        Añadir
                                    </button>
                                </div>
                            </div>

...
class VentaForm
{
    ...
    ->label('Cliente')
    ->relationship('cliente', 'nombre') // Asegúrate que en la tabla 'clientes' la columna se llame 'nombre'
    ->searchable()
    ->preload() // ESTO es lo que hace que la lista aparezca al hacer click
    ->required()
    ->columnSpan(7),
...
Select::make('user_id')
    ...
    ->default(auth()->id())
    ->disabled() // Lo mantenemos visualmente desactivado
    ->dehydrated() // ¡ESTO ES LO IMPORTANTE! Fuerza a Filament a enviar el ID al guardar
    ->required()
    ->columnSpan(5),
                // --- CUERPO (Línea 2) ---
                ...
                Repeater::make('detalles')
                    ...
                    ->columns(4) // Columnas dentro de cada fila del producto
                    ->columnSpan(8) // Ancho del bloque completo de productos
                ...
                TextInput::make('total_venta')
                    ...
                    ->prefix('$')
                    ->readonly()
                    ->columnSpan(4) // Esto lo empuja al lado del Repeater
